...
sto_path = "/data7/hasnain/cmf_v420_pkg/out/tej_vic_nolev/"
...
try:
    ...
    sto_path = CFG.get("sto_path_nat", sto_path)
    ...
    nat_rps = RPS.get("RPs")
    if nat_rps is not None:
        if isinstance(nat_rps, (list, tuple)):
            fixed_return_periods = [int(x) for x in nat_rps]
        else:
            fixed_return_periods = [int(x) for x in str(nat_rps).split(",") if x]
...
output_csv =  os.path.join(output_dir, f"nat_Storage_Stats_with_Distributions_all_grids.csv")
stats_df.to_csv(output_csv, index=False)
print(f"Statistics saved to {output_csv}")

# Save binary results for each return period
for rp, binary_result in binary_results.items():
    binary_output_filename = os.path.join(output_dir, f"nat_storage_RP{rp}.bin")
